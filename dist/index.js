...
async function getConfig(guildId) {
    ...
    if (cached && Date.now() < cached.exp)
        return cached.data;
    try {
        const res = await api_1.api.getPanel(guildId);
...
function statusOf(k) {
    if (k.isLocked)
        return "locked";
    if (!k.isLifetime && k.expiresAt && new Date(k.expiresAt) < new Date())
        return "expired";
    return "active";
}
...
function keyEmbed(k) {
    ...
    return new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: "Key ID", value: "`" + k.id + "`", inline: false }, { name: "Project", value: k.project ?? "—", inline: true }, { name: "Status", value: status, inline: true }, { name: "Expiry", value: daysLabel(k), inline: true }, { name: "Executions", value: String(k.executions), inline: true }, { name: "HWID", value: k.hwid ? "`" + k.hwid.slice(0, 16) + "`" : "unbound", inline: true }, { name: "Discord", value: k.discordId ? `<@${k.discordId}>` : "—", inline: true }, { name: "Note", value: k.note || "—", inline: false });
}
// Inject the buyer's key into the configured loader text (or fall back to a
// generated loadstring snippet).
function buildLoader(cfg, rawKey) {
    const key = rawKey || "YOUR_KEY_HERE";
    if (cfg?.loaderText) {
        const replaced = cfg.loaderText.replace(/YOUR_KEY_HERE|YOUR_KEY|%KEY%|\{KEY\}|\{key\}/g, key);
        if (replaced !== cfg.loaderText || /script_key/i.test(replaced))
            return replaced;
...
async function grantRole(guildId, userId, roleId) {
    if (!roleId)
        return;
    await client.rest.put(discord_js_1.Routes.guildMemberRole(guildId, userId, roleId));
}
async function removeRole(guildId, userId, roleId) {
    if (!roleId)
        return;
    await client.rest.delete(discord_js_1.Routes.guildMemberRole(guildId, userId, roleId)).catch(() => { });
}
function memberHasRole(i, roleId) {
...
async function resolveProjectId(i) {
...
async function isManager(i) {
    if (OWNER_IDS.includes(i.user.id))
        return true;
    const perms = i.memberPermissions;
    if (perms && (perms.has(discord_js_1.PermissionFlagsBits.Administrator) || perms.has(discord_js_1.PermissionFlagsBits.ManageGuild)))
        return true;
    if (i.guildId) {
        const cfg = await getConfig(i.guildId);
        if (cfg?.managerRoleId && memberHasRole(i, cfg.managerRoleId))
            return true;
    }
    return false;
}
async function replyText(i, content, color = ACCENT) {
    const embed = new discord_js_1.EmbedBuilder().setColor(color).setDescription(content);
    if (i.deferred || i.replied)
        await i.editReply({ embeds: [embed] });
    else
        await i.reply({ embeds: [embed], flags: discord_js_1.MessageFlags.Ephemeral });
}
/* ----------------------------- Panel ----------------------------- */
function buildPanel() {
    ...
    const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId("panel_redeem").setLabel("Redeem Key").setEmoji("🔑").setStyle(discord_js_1.ButtonStyle.Success), new discord_js_1.ButtonBuilder().setCustomId("panel_script").setLabel("Get Script").setEmoji("📜").setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId("panel_role").setLabel("Get Role").setEmoji("👤").setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId("panel_resethwid").setLabel("Reset HWID").setEmoji("⚙️").setStyle(discord_js_1.ButtonStyle.Secondary), new discord_js_1.ButtonBuilder().setCustomId("panel_stats").setLabel("Get Stats").setEmoji("📊").setStyle(discord_js_1.ButtonStyle.Secondary));
    return { embeds: [embed], components: [row] };
}
// Buyer keys are matched across all projects (universal bot — no project binding).
async function findBuyerKey(discordId) {
    const res = await api_1.api.lookup({ discordId });
    return res.keys[0] ?? null;
}
/* --------------------- Buyer button handlers --------------------- */
async function handleButton(i) {
    if (i.customId === "panel_redeem") {
        const modal = new discord_js_1.ModalBuilder().setCustomId("redeem_modal").setTitle("Redeem Key");
    ...
    try {
        ...
        if (i.customId === "panel_resethwid") {
            ...
            await api_1.api.manage({ action: "reset_hwid", keyId: k.id, adminId: i.user.id });
        ...
        if (i.customId === "panel_stats") {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .addFields({ name: "Status", value: statusOf(k), inline: true }, { name: "Expiry", value: daysLabel(k), inline: true }, { name: "Executions", value: String(k.executions), inline: true }, { name: "HWID", value: k.hwid ? "bound" : "unbound", inline: true });
            await i.editReply({ embeds: [embed] });
            return;
        }
    }
...
async function handleModal(i) {
    if (i.customId !== "redeem_modal")
        return;
    await i.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
    const rawKey = i.fields.getTextInputValue("key").trim();
    try {
        const res = await api_1.api.lookup({ key: rawKey });
        const k = res.keys[0];
        if (!k) {
        ...
        await api_1.api.manage({ action: "set_discord", keyId: k.id, discordId: i.user.id, adminId: i.user.id });
        const cfg = i.guildId ? await getConfig(i.guildId) : null;
        if (i.guildId && cfg?.buyerRoleId)
            await grantRole(i.guildId, i.user.id, cfg.buyerRoleId).catch(() => { });
...
async function handleCommand(i) {
    ...
    try {
        switch (i.commandName) {
            case "setpanel": {
                if (!i.guildId)
                    return void (await replyText(i, "Use this in a server.", RED));
                ...
                if (!loaderText.trim())
                    return void (await replyText(i, "The attached file is empty.", RED));
                await api_1.api.setPanel({
                    guildId: i.guildId,
            ...
            case "login": {
                ...
                const res = await api_1.api.login({ guildId: i.guildId, apiKey });
                invalidateConfig(i.guildId);
                await replyText(i, `✅ Linked to **${res.username}**${res.projectName
                    ? ` · default project: **${res.projectName}**`
                    : " — no projects yet, create one on the dashboard"}.`, GREEN);
                break;
            }
            case "generate":
            case "mass-generate": {
                ...
                if (!projectId)
                    return void (await replyText(i, "No project linked. Run `/login` first or pass `project_id`.", RED));
                ...
                const res = await api_1.api.generate({
                    projectId,
                    count,
                    durationDays: days,
                    isLifetime: lifetime,
                ...
                await replyText(i, `Generated **${res.keys.length}** key${res.keys.length !== 1 ? "s" : ""}:\n\`\`\`\n${list}\n\`\`\``, GREEN);
                break;
            }
            case "lookup": {
                ...
                if (!key && !user)
                    return void (await replyText(i, "Provide a key or a user.", RED));
                const res = await api_1.api.lookup({ key, discordId: user?.id, projectId });
                if (res.keys.length === 0)
                    return void (await replyText(i, "No matching keys found.", RED));
            ...
            case "whitelist": {
                ...
                await api_1.api.manage({ action: "set_discord", keyId: created.id, discordId: user.id, adminId: i.user.id });
                const cfg = i.guildId ? await getConfig(i.guildId) : null;
                if (i.guildId && cfg?.buyerRoleId)
                    await grantRole(i.guildId, user.id, cfg.buyerRoleId).catch(() => { });
            ...
            case "unwhitelist": {
                const user = i.options.getUser("user", true);
                const res = await api_1.api.lookup({ discordId: user.id });
                if (res.keys.length === 0)
                    return void (await replyText(i, "No keys linked to that user.", RED));
                for (const k of res.keys) {
                    await api_1.api.manage({ action: "lock", keyId: k.id, lockReason: "unwhitelisted", adminId: i.user.id });
                }
                const cfg = i.guildId ? await getConfig(i.guildId) : null;
                if (i.guildId && cfg?.buyerRoleId)
                    await removeRole(i.guildId, user.id, cfg.buyerRoleId);
            ...
            case "blacklist": {
                ...
                if (res.keys.length === 0)
                    return void (await replyText(i, "No keys linked to that user.", RED));
                for (const k of res.keys) {
                    await api_1.api.manage({ action: "lock", keyId: k.id, lockReason: reason, adminId: i.user.id });
                }
                const cfg = i.guildId ? await getConfig(i.guildId) : null;
                if (i.guildId && cfg?.buyerRoleId)
                    await removeRole(i.guildId, user.id, cfg.buyerRoleId);
            ...
            case "lock": {
                ...
                await api_1.api.manage({ action: "lock", keyId, lockReason: reason, adminId: i.user.id });
                await replyText(i, `Key \`${keyId}\` locked.`, RED);
                break;
            }
            case "unlock": {
                const keyId = i.options.getString("key_id", true);
                await api_1.api.manage({ action: "unlock", keyId, adminId: i.user.id });
                await replyText(i, `Key \`${keyId}\` unlocked.`, GREEN);
                break;
            }
            case "stats": {
                const projectId = i.options.getString("project_id") ?? undefined;
                const s = await api_1.api.stats(projectId);
                const embed = new discord_js_1.EmbedBuilder()
                    .setColor(ACCENT)
                    .setTitle(`Stats — ${s.project ?? "All projects"}`)
                    .addFields({ name: "Total keys", value: String(s.total), inline: true }, { name: "Active", value: String(s.active), inline: true }, { name: "Locked", value: String(s.locked), inline: true }, { name: "Expired", value: String(s.expired), inline: true }, { name: "Executions", value: s.executions.toLocaleString(), inline: true });
                await i.editReply({ embeds: [embed] });
                break;
            }
            default:
...
function errMessage(err) {
    ...
    if (err instanceof Error)
        return err.message + causeOf(err);
...
client.once(discord_js_1.Events.ClientReady, async (c) => {
    ...
    try {
        ...
        console.error(`[bastion-bot] connectivity FAILED to ${BASE}: ${e.message}${causeOf(e)}`);
    }
});
